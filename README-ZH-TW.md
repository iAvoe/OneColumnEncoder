...
1. **x265 壓力測試預設**：提供用於驗證系統穩定性的 x265 壓力測試預設。但該測試的實際負載取決於輸入影片的內容複雜度，建議使用與目標壓製任務一致的測試影片作為源文件進行驗證。
  - 由於該測試的壓力可能會高於 Prime95 等傳統壓力測試工具，因此運行該測試本身就存在一定風險，只建議在全程監控溫度和系統狀態的前提下運行，並且在開始測試前先做好數據備份。
...
既然底層已經鎖死在 Windows API 上，選擇 WPF 是最自然的結果——它提供了原生 Windows 桌面集成（包括防止打開溢出畫面窗口的功能）、成熟的 MVVM 數據綁定生態，且無需引入瀏覽器內核或第三方依賴。跨平台框架（如 Avalonia / MAUI / Qt）解決的是 UI 層的移植問題，但無法解決底層 API 的相容，反而會增加一層抽象成本和測試負擔。
