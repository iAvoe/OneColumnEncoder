...
- `EncoderConfM.CustomParams` 会被保存，但当前没有被 `EncodingPipelineH` 拼入最终编码命令。
- “自定义参数”区域目前更像第三方开关汇总，没有真正的自由文本参数直通。
- x264 / x265 / SVT-AV1 的参数覆盖范围仍有限，不是完整编码器参数编辑器。

### 并行与内存策略

- NUMA 节点、物理核心优先和编码器线程数已接入，但 P-Core / E-Core 专门分配仍未实现。
- Large Pages 目前只有权限检查、配置字段和 UI 开关，没有实际影响进程启动或编码器内存分配。
- SVT-AV1 尚未根据并行配置生成线程数参数。
