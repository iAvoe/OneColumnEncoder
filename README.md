...
- 多语言资源已经覆盖大量 UI 文本，但仍存在硬编码英文标题、调试标题和提示文本。
- 部分并行设置文案仍直接带 `TODO`。
...
- README 中尚未包含构建、运行、依赖工具准备和典型工作流说明。

## 完全没做

### Best Practices 检查卡

- `BestPracsCardVM` 当前只填充清单文本。
- 没有实际检测逻辑，也没有参与编码启动前的阻塞条件。
...
- `Commands/SampleClipCmd.cs` 只显示 sample command 文本，没有启动采样编码。
- 主流程实际使用的是 `OpenSampleClipCmd` 和 `SampleClipModalVM`，旧命令可视为未接线代码。
