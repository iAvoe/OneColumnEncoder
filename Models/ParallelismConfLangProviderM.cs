...
["UpstreamNumaTitle"] = "Vinculación NUMA suave: aguas arriba del pipeline",
            ["DownstreamNumaTitle"] = "Vinculación NUMA suave: aguas abajo del pipeline (codificador)",
            ["NumaGuidanceText"] = "Asignar el codificador a otro nodo puede ser más rápido si la herramienta aguas arriba usa filtros lentos;\nsi no, compartir nodo puede ser más rápido — cuello de botella de cómputo frente a latencia.",
        ...
        ["ru"] = new()
        {
            ...
            ["CacheGroupTitle"] = "Обнаруженные группы L3-кэша (↑ переходы между группами, ↓ попадания в кэш)",
...
["UpstreamNumaTitle"] = "Мягкая NUMA-привязка: апстрим конвейера",
            ["DownstreamNumaTitle"] = "Мягкая NUMA-привязка: даунстрим конвейера (кодер)",
            ["NumaGuidanceText"] = "Кодер на другом узле может быть быстрее, если апстрим использует медленные фильтры;\nиначе общий узел может быть быстрее — вычислительное узкое место против задержки.",
