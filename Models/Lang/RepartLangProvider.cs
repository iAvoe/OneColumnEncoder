...
public sealed class RepartLangProvider(string languageCode) : LangProviderBase(languageCode, Data)
{
    ...
    private static readonly Dictionary<string, Dictionary<string, string>> Data = new()
    {
        ["en"] = new()
        {
            ...
            ["PlaylistRemoveFromFinal"] = "Remove",
            ...
            ["TimelineHintDetailed"] = "This mode is for conjoined discs (under/over-segmented), or for sources w/ missed episode divisions. This mode needs manual tuning. Chapter-mode import will create excess dividers for OP, ED, etc.; delete them to fully correct episode partitions",
        ...
        ["zh-cn"] = new()
        {
            ["Tool.Source.VideoSrcRepart"] = "📁 视频来源重分集",
            ["SelectFolder"] = "选择重分集视频来源资料夹",
            ...
            ["PlaylistRemoveFromFinal"] = "移除",
            ["PlaylistClearAll"] = "全部移除",
            ["PlaylistConfirm"] = "确认",
            ["ChapterSourcesMissing"] = "章节档案夹未能解析出存在的视频来源档案。请确认播放清单引用的媒体档案（m2ts）存在",
            ...
            ["StageCheckFiles"] = "正在检查视频来源档案……",
            ["StageProbeFiles"] = "正在用 ffprobe 探测视频来源……",
            ["StageAnalyzeStreams"] = "正在分析视频串流……",
            ["StageValidateStreams"] = "正在验证视频串流……",
            ["StageScanFrames"] = "正在扫描影格数……",
            ["ExcludedCount"] = "已排除 {0} 个不相容来源",
            ["Ready"] = "视频源格式一致，可以分集",
            ["ReadyWithExcluded"] = "视频源格式一致，已排除 {0} 个不相容来源项目",
            ["InterlacedSourcePrompt"] = "重分集模式要求逐行扫描、可稳定扫描帧数的 CFR 来源，以确保按帧精确输出。\n\n确认：丢弃此来源并继续汇入。\n取消：取消本次汇入",
            ["InterlacedSrcRejected"] = "重分集模式不支援隔行扫描来源：{0}（field_order={1}）",
            ["FrameCountFallbackPrompt"] = "ffmpeg 不可用，无法精确取得帧数，且无法验证视频来源 {0} 的总帧数（估算约 {1:N0} 帧）。",
            ["FrameCountFallbackPromptChoices"] = "确认：继续尝试搜寻，每一步把帧数范围扩大 10 倍。\n取消：取消本次汇入",
            ["FrameCountFallbackCancelled"] = "已取消对 {0} 的帧数搜寻",
            ["Summary"] = "{0} 个视频来源 | {1:N0} 帧 | {2}/{3} fps | {4}",
            ["FfprobeRequired"] = "重分集模式需要 ffprobe",
            ["FfmpegRequired"] = "重分集模式需要 ffmpeg 以产生 video-only MKV",
            ["SourceRequired"] = "请至少汇入一个视频来源",
            ["MinFolderSources"] = "所选资料夹必须至少包含两个视频来源档案",
            ["MinSourcesRequired"] = "重分集模式要求筛选后至少保留两个可用视频来源",
            ["SourceMissing"] = "视频来源档案不存在：{0}",
            ["SourceChanged"] = "视频来源在分析过程中发生了变化：{0}",
            ["StalePlanSourceChanged"] = "一个或多个重分集视频来源已变更或无法使用。请重新汇入来源资料夹以重建影格时间轴",
            ...
            ["SignatureMismatch"] = "视频来源 {0} 与参考视频串流格式不完全一致",
            ["ProbeFailed"] = "ffprobe 执行失败或没有传回资料",
            ["InvalidRange"] = "请输入位于虚拟时间轴内的有效时间或帧范围",
            ["Overlap"] = "输出分集不能相互重叠",
            ["UniqueName"] = "输出分集名称必须有效且不能重复",
            ["OutputsRequired"] = "套用前请至少新增一个输出分集",
            ["RevisionDisabled"] = "重分集计划不允许使用视频来源修订；如需修改来源资讯，请重新汇入视频来源",
            ["FrameChangingFiltersWarning"] = "重分集输出按来源影格号精确切分。请避免使用会改变影格数或影格时序的滤镜，例如 IVTC、抽帧、补帧、CFR/VFR 转换",
            ["SrcLabel"] = "视频来源：{0}",
            ["WillExcludeSource"] = "将排除此视频来源，关闭对话方块后继续处理其余项目",
            ["ImportSummary"] = "重分集汇入完成：已接受 {0} 个视频来源，已排除 {1} 个来源项目",
    ...
    static RepartLangProvider()
    {
        Data["zh-tw"] = new(Data["zh-cn"])
        {
            ...
            ["DividerPreviewFfmpegUnavailable"] = "ffmpeg 不可用，無法生成預覽",
        ...
        Data["fr"] = new(Data["en"])
        {
            ...
            ["TimelineHintDetailed"] = "Ce mode traite les disques joints (sous/sur-segmentés) ou les épisodes mal découpés. Réglage manuel requis. L'import par chapitres crée des séparateurs en trop (OP, ED, ...) : supprimez-les pour ajuster le découpage",
            ["DividerPreviewSelectDivider"] = "Sélectionnez un séparateur pour afficher l'aperçu",
            ["DividerPreviewFfmpegUnavailable"] = "ffmpeg est indisponible, l'aperçu ne peut pas être généré",
        ...
        Data["es"] = new(Data["en"])
        {
            ...
            ["PlaylistRemoveFromFinal"] = "Quitar",
            ...
            ["TimelineHintDetailed"] = "Este modo es para discos unidos (sub/sobresegmentados) o cortes de episodios erróneos. Se requiere ajuste manual. La import. por capítulos crea divisores innecesarios (OP, ED, ...): elimínelos para corregir la partición",
            ["DividerPreviewSelectDivider"] = "Selecciona una división para mostrar la vista previa",
            ["DividerPreviewFfmpegUnavailable"] = "ffmpeg no está disponible, no se puede generar la vista previa",
        ...
        Data["ja"] = new(Data["en"])
        {
            ...
            ["DividerPreviewFfmpegUnavailable"] = "ffmpeg が利用できないため、プレビューを生成できません",
        ...
        Data["ru"] = new(Data["en"])
        {
            ...
            ["PlaylistRemoveFromFinal"] = "Удалить",
            ...
            ["TimelineHintDetailed"] = "Режим для объединенных дисков (недо-/пере-сегментация) и ошибок разбивки. требуется ручная настройка. Импорт по главам создает лишние разделители (OP, ED, ...) — удалите их для точной разметки",
            ["DividerPreviewSelectDivider"] = "Выберите разделитель для просмотра предпросмотра",
            ["DividerPreviewFfmpegUnavailable"] = "ffmpeg недоступен, поэтому предпросмотр невозможно создать",
        ...
        Data["de"] = new(Data["en"])
        {
            ...
            ["PlaylistRemoveFromFinal"] = "Entfernen",
            ...
            ["TimelineHintDetailed"] = "Modus für verbundene Disks (Unter-/Übersegmentierung) oder fehlerhafte Episodentrennung. Manuelle Abstimmung erforderlich. Kapitelimport erzeugt überzählige Trennlinien (OP, ED, ...) — löschen für korrekte Aufteilung",
            ["DividerPreviewSelectDivider"] = "Trennlinie für Vorschau auswählen",
            ["DividerPreviewFfmpegUnavailable"] = "ffmpeg nicht verfügbar, Vorschau kann nicht erstellt werden",
        ...
        Data["ko"] = new(Data["en"])
        {
            ...
            ["DividerPreviewFfmpegUnavailable"] = "ffmpeg를 사용할 수 없어 미리보기를 생성할 수 없습니다",
        ...
        Data["pt-br"] = new(Data["en"])
        {
            ...
            ["PlaylistRemoveFromFinal"] = "Remover",
            ...
            ["TimelineHintDetailed"] = "Este modo é para discos conjugados (sub/sobre-segmentados) ou para fontes com divisões de episódios perdidas. Ajuste manual necessário. A importação por capítulos criará divisórias excessivas para OP, ED, etc.; exclua-as para corrigir totalmente as partições de episódios",
            ["DividerPreviewSelectDivider"] = "Selecione uma divisória para exibir a visualização",
            ["DividerPreviewFfmpegUnavailable"] = "ffmpeg indisponível, visualização não pode ser gerada",
