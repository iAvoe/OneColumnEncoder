...
public sealed class RepartLangProvider(string languageCode)
{
    private static readonly Dictionary<string, Dictionary<string, string>> Data = new()
    {
        ["en"] = new()
        {
            ...
            ["ChapterFileImportPrompt"] = "Import Repart as a chapter file?\n\nConfirm: pick a chapter file (.mpls / .ifo / .xpl) to read episode boundaries.\nCancel: import a video source folder as usual",
            ["SelectChapterFile"] = "Select a chapter file (.mpls / .ifo / .xpl)",
            ["ChapterSourcesMissing"] = "The chapter file did not resolve to at least two existing video source files. Make sure the media files (m2ts) referenced by the chapter file are present next to it",
        ...
        ["zh-cn"] = new()
        {
            ...
            ["ChapterFileImportPrompt"] = "是否以章节文件形式导入？\n\n确认：选择章节文件（.mpls / .ifo / .xpl）读取分集边界。\n取消：按常规方式导入视频源文件夹",
            ["SelectChapterFile"] = "选择章节文件（.mpls / .ifo / .xpl）",
            ["ChapterSourcesMissing"] = "章节文件未能解析出至少两个存在的视频源文件。请确认章节文件引用的媒体文件（m2ts）位于其旁边",
    ...
    static RepartLangProvider()
    {
        Data["zh-tw"] = new(Data["zh-cn"])
        {
            ...
            ["ChapterFileImportPrompt"] = "是否以章節檔案形式匯入？\n\n確認：選擇章節檔案（.mpls / .ifo / .xpl）讀取分集邊界。\n取消：按常規方式匯入影片來源資料夾",
            ["SelectChapterFile"] = "選擇章節檔案（.mpls / .ifo / .xpl）",
            ["ChapterSourcesMissing"] = "章節檔案未能解析出至少兩個存在的影片來源檔案。請確認章節檔案引用的媒體檔案（m2ts）位於其旁",
        ...
        Data["fr"] = new(Data["en"])
        {
            ...
            ["ChapterFileImportPrompt"] = "Importer la répartition via un fichier de chapitres ?\n\nConfirmer : choisir un fichier de chapitres (.mpls / .ifo / .xpl) pour lire les limites d'épisodes.\nAnnuler : importer un dossier de sources vidéo comme d'habitude",
            ["SelectChapterFile"] = "Sélectionner un fichier de chapitres (.mpls / .ifo / .xpl)",
            ["ChapterSourcesMissing"] = "Le fichier de chapitres n'a pas résolu au moins deux fichiers sources vidéo existants. Vérifiez que les fichiers média référencés (m2ts) sont présents à côté du fichier de chapitres",
        ...
        Data["es"] = new(Data["en"])
        {
            ...
            ["ChapterFileImportPrompt"] = "¿Importar Repart como archivo de capítulos?\n\nConfirmar: elegir un archivo de capítulos (.mpls / .ifo / .xpl) para leer los límites de episodios.\nCancelar: importar una carpeta de fuentes de vídeo como siempre",
            ["SelectChapterFile"] = "Seleccionar archivo de capítulos (.mpls / .ifo / .xpl)",
            ["ChapterSourcesMissing"] = "El archivo de capítulos no resolvió al menos dos archivos de fuente de vídeo existentes. Asegúrate de que los archivos multimedia referenciados (m2ts) estén junto al archivo de capítulos",
        ...
        Data["ja"] = new(Data["en"])
        {
            ...
            ["ChapterFileImportPrompt"] = "チャプターファイルとして再分割をインポートしますか？\n\n確認: チャプターファイル（.mpls / .ifo / .xpl）を選択してエピソード境界を読み取ります。\nキャンセル: 通常どおり映像ソースフォルダーをインポートします",
            ["SelectChapterFile"] = "チャプターファイルを選択（.mpls / .ifo / .xpl）",
            ["ChapterSourcesMissing"] = "チャプターファイルから既存の映像ソースファイルを 2 つ以上解決できませんでした。チャプターファイルの隣に参照メディアファイル（m2ts）があることを確認してください",
        ...
        Data["ru"] = new(Data["en"])
        {
            ...
            ["ChapterFileImportPrompt"] = "Импортировать разбивку из файла глав?\n\nПодтвердить: выбрать файл глав (.mpls / .ifo / .xpl), чтобы прочитать границы эпизодов.\nОтмена: импортировать папку с исходными видео как обычно",
            ["SelectChapterFile"] = "Выберите файл глав (.mpls / .ifo / .xpl)",
            ["ChapterSourcesMissing"] = "Файл глав не дал как минимум двух существующих файлов исходных видео. Убедитесь, что ссылки на медиафайлы (m2ts) находятся рядом с файлом глав",
