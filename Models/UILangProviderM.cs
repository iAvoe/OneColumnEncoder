...
public class UILangProviderM
{
    ...
    private static readonly Dictionary<string, Dictionary<string, string>> Data = new()
    {
        ["en"] = new()
        {
            ...
            ["EncInspect.P1Text"] = "Battery / AC Power:\nDetects whether the computer is running on battery power. Encoding on battery may cause instability or shutdown. Checks via SystemInformation.PowerStatus.",
            ["EncInspect.P1Title"] = "Battery / Off-Grid Check",
            ["EncInspect.P2Text"] = "Disk Space:\nCompares available disk space to source file size × 1.5 (fallback: 1 GiB). Ensures enough room for encoding output.",
            ["EncInspect.P2Title"] = "Disk Space Check",
            ["EncInspect.P3Text"] = "Write Permission:\nVerifies write access to the output directory by attempting to list files. Prevents failed encoding due to permission errors.",
            ["EncInspect.P3Title"] = "Write Permission Check",
            ["EncInspect.P4Text"] = "Overwrite:\nChecks if the output file already exists. Warns before overwriting to prevent accidental data loss.",
            ["EncInspect.P4Title"] = "Overwrite Check",
            ["EncInspect.P5Text"] = "L-SMASH Plugin:\nWhen Avs2Yuv upstream is selected, verifies that libvslsmashsource.dll exists under the AviSynth+ plugin path. Required for L-SMASH Works to read source frames.",
        ...
        ["zh-cn"] = new()
        {
            ...
            ["EncInspect.P1Text"] = "电池 / 供电状态：\n检测电脑是否使用电池供电。在电池模式下压制可能引发不稳定或意外关机。通过 SystemInformation.PowerStatus 检查。",
            ["EncInspect.P1Title"] = "电池 / 离网检查",
            ["EncInspect.P2Text"] = "磁盘空间：\n比较可用磁盘空间与源文件大小 × 1.5（回退值：1 GiB）。确保有足够的空间存放压制结果。",
            ["EncInspect.P2Title"] = "磁盘空间检查",
            ["EncInspect.P3Text"] = "写入权限：\n尝试列出输出目录的文件以验证写入权限。防止因权限不足导致压制失败。",
            ["EncInspect.P3Title"] = "写入权限检查",
            ["EncInspect.P4Text"] = "覆盖检查：\n检查输出文件是否已存在。覆盖前提醒，避免意外数据丢失。",
            ["EncInspect.P4Title"] = "覆盖检查",
            ["EncInspect.P5Text"] = "L-SMASH 插件：\n当选择 Avs2Yuv 上游时，验证 libvslsmashsource.dll 是否存在于 AviSynth+ 插件路径下。L-SMASH Works 需此插件才能读取源帧。",
        ...
        ["zh-tw"] = new()
        {
            ...
            ["EncInspect.P1Text"] = "電池 / 供電狀態：\n檢測電腦是否使用電池供電。在電池模式下壓制可能引發不穩定或意外關機。通過 SystemInformation.PowerStatus 檢查。",
            ["EncInspect.P1Title"] = "電池 / 離網檢查",
            ["EncInspect.P2Text"] = "磁碟空間：\n比較可用磁碟空間與源文件大小 × 1.5（回退值：1 GiB）。確保有足夠的空間存放壓制結果。",
            ["EncInspect.P2Title"] = "磁碟空間檢查",
            ["EncInspect.P3Text"] = "寫入權限：\n嘗試列出輸出目錄的文件以驗證寫入權限。防止因權限不足導致壓制失敗。",
            ["EncInspect.P3Title"] = "寫入權限檢查",
            ["EncInspect.P4Text"] = "覆蓋檢查：\n檢查輸出文件是否已存在。覆蓋前提醒，避免意外數據丟失。",
            ["EncInspect.P4Title"] = "覆蓋檢查",
            ["EncInspect.P5Text"] = "L-SMASH 插件：\n當選擇 Avs2Yuv 上游時，驗證 libvslsmashsource.dll 是否存在於 AviSynth+ 插件路徑下。L-SMASH Works 需此插件才能讀取源幀。",
