...
| Concat | Analyze all concat files. ffprobe failures still reject the batch, but VFR and frame-rate mismatches now surface as warnings through `OpenWarnModalCmd` and do not block import. Concat additionally sums every fragment's frame count into a **concat total frame count** only when every fragment has a real frame count. |
...
The summed frame count (computed in `ConcatCompatibilityAnalyzer.AnalyzeAsync()`) is stored in `VideoAnalysisM.ConcatTotalFrames` and later forwarded into `EncodingPipelineRequest.ConcatTotalFrames`. This value takes priority over the first fragment's individual frame count in `EncodingPipeline.GetSourceTotalFrames()`, is used by `EncodingPipeline.BuildAutoGeneratedEncoderParams()` for encoder frame-count arguments (`--frames` / `-n`) only when it is known, and is consumed by `EncodingMonitorVM` (progress bar) and `QueueJobItemVM` (frame count label).
...
During auto encoder parameter generation, concat requests pass `ConcatTotalFrames` into `GetFrameCount()`. This prevents x264/x265/SVT-AV1 from receiving only the first fragment's `nb_frames` as the total frame count.
