...
| Concat | Analyze all concat files. ffprobe failures still reject the batch, but VFR and frame-rate mismatches now surface as warnings through `OpenWarnModalCmd` and do not block import. Concat additionally sums every fragment's frame count into a **concat total frame count**. |
