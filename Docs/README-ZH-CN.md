...
- **编码器参数配置**
    - 提供方便的 UI 控件来按符合编码器开发者意图的客制化视频压制参数配置
...
- **高级监控**
    - 监控每个应用程序的内存使用情况，分别查看上游和下游（编码器）日志
- **中断控制**
    - 可中断上游或下游程序（编码器）以一致地退出编码会话
- **覆写保护**
    - 基于待覆盖文件大小的冷却时间防呆设计
...
- [x] OpenCL 兼容性检查
- [x] VapourSynth-zipcl 已集成，且提供了部分自动生成的导入、降噪、去色带和模糊滤镜示例
- [x] VapourSynth 预览器已实现，界面和压制预览器一致
- [ ] 利用 Nuget 包插件列出 OpenCL 设备？
- [ ] 验证队列模式、合并模式下使用扩展滤镜的压制结果
