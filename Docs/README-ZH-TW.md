...
**高級平行處理**
- 在指定執行緒數量的基礎上，支持逐物理核心和 NUMA 節點綁定設置

**編碼器壓制參數配置**
- 提供方便的 UI 控制項來配置影片壓製參數
- 提供單圖壓制 A/B 對比預覽器

**影片濾鏡配置**
- 提供 FFMPEG-VS-AVS 濾鏡編輯器
- 自動編寫基本的 AviSynth、VapourSynth 腳本（解碼→Y4M）
- 為 FFMPEG-VS-AVS 生成多種濾鏡：
  - VFR→CFR
  - HDR→SDR（WCG）
  - 色彩空間轉換
  - SAR 修復
...
- 支持高級壓制模式：
  - **隊列模式**：一次進行一系列影片源的濾鏡處理和壓製
  - **拼接模式/合併模式**: 一次進行多個影片源的拼接、濾鏡處理和壓制
  - **重分集模式**: 一次進行多個影片源的虛擬重組與再切割，以修正合併盤與肉醬盤，或誤切分的影片源

在合併模式中，如果選擇 AAC 或 Opus 這類音訊重編碼，流程會把音訊獨立拆成一條 ffmpeg 壓製命令，再接一條最終封裝命令，命令列表會變成「影片壓製 -> 音訊壓製 -> 封裝」。

**可編輯藍光章節導入**
- 直接導入 BDMV 結構下的 PLAYLIST 列表，解析並自訂藍光章節選取

**取段打樣**
- 基於 FFMPEG-VS-AVS 命令行實現的片段截取壓製
- 提供方便截取時間或幀號段的操作界面
...
- [GitHub 發布頁](https://github.com/iAvoe/OneColumnEncoder/releases)
...
- 藍光播放列表解析器 by [tautcony](https://github.com/tautcony): [ChapterTool Core](https://github.com/tautcony/ChapterTool/pkgs/nuget/ChapterTool.Core)
...
- 源檢查卡已經解析並展示 progressive、位深、幀率、SAR、色彩元數據、chroma、HDR / Dolby Vision 源資訊等檢查項
...
- ffprobe 自動參數現在也會在中繼資料存在時補充 HDR mastering display 與 content light level；源檢查副標題會直接顯示 HDR / Dolby Vision 源資訊

#### 編碼命令生成與啟動

- Y4M 管線命令生成已經實現，支持多種上游工具輸出到 x264 / x265 / SVT-AV1
- 命令生成會結合 ffprobe 資訊自動補充部分幀數、色彩、range、chroma、lookahead、HDR mastering display、content light level 等參數
...
#### 多開（Run multiple instances）

- 單個 1cenc 實例裡，下游編碼器執行緒一次只能綁定到一個 NUMA 節點；如果壓制任務數量較多，而機器還有空餘 NUMA 節點，可以啟動與可用 NUMA 節點數量相同的程序實例，並讓每個實例各自處理隊列中的一部分任務，從而把所有節點都用滿。
- 區分這些實例很簡單，只要看標題欄或任務欄裡的 `1cenc · PID xxxxx · vX.Y.Z` 即可。PID 是進程的臨時編號，在運行期間唯一，不同實例不會重複。
...
- 源附帶字幕軌道會在封裝編輯器中保留 ffprobe 提取的語言和時長元數據
- 導入的字幕文件會被校驗，其最後一個字幕片段的結束時間會作為軌道時長
- 源附帶的字幕軌道和導入的字幕文件有分色顯示功能
- 已測試：調整源附帶字幕“默認”的命令行生成效果正常
- 已測試：源附帶字幕的元數據解析全部正常
- 已測試：ISO 639-2 語言代碼識別正常
- 已測試：隊列模式
- 已測試：合併模式
...
- 源分析和檢查結果提示：`Commands/AnalyzeSrcVideoCmd.cs`、`Commands/OpenRawAnalysisCmd.cs`、`Commands/InspectEncProblemsCmd.cs`、`Commands/InspectSrcProblemsCmd.cs`
