...
**Filter placement rule.** Imported sources are never modified. Repart Filter Scribe uses the current Repart source queue to build a temporary concat-style source and applies the selected filters to the Repart stream; the queue may be reordered without changing committed frame boundaries. Source Reviser remains disabled while a Repart plan is active because changing source metadata requires rebuilding the plan. Repart ffmpeg requests retain `FfmpegFilterArgs`, while AVS/VPY execution scripts include the saved Filter Scribe body.
