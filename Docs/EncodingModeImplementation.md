...
On success, the first fragment's raw ffprobe JSON is stored in `_srcVideoAnalysis.RawJson` and drives encoder config previews and FilterScribe helpers. The full per-fragment analysis set is serialized into `_srcVideoAnalysis.BatchRawJson` so `OpenRawAnalysisCmd` can open all fragment JSON.
...
Clicking `SrcRepart` opens an import flow that can read either a plain STREAM folder or a chapter-folder/PLAYLIST folder. The sources are analyzed in picker/playlist order before the Repart configuration modal opens. The Repart configuration modal owns both source and output queue editing: source ordering/removal rebuilds the virtual source offsets, while output ordering/removal changes the encoding task list without changing input sources or frame ranges. At least one source and one output task must remain. The modal also contains a proportional partition map and synchronized time/frame fields. Output ranges use inclusive first/last frames; ranges cannot overlap, gaps are allowed, and merge accepts only directly adjacent outputs.
...
**Filter placement rule.** Imported sources are never modified. Repart Filter Scribe uses the Repart source list internally to build a temporary concat-style source and apply the selected filters. Source Reviser remains disabled while a Repart plan is active because changing source metadata requires rebuilding the plan. Repart ffmpeg requests retain `FFmpegFilterArgs`, while AVS/VPY execution scripts include the saved Filter Scribe body.
...
**Repart:** `SrcRepart` selected → import a folder → strictly analyze sources in import order → open `RepartConfModal` → optionally reorder/remove input sources → manually allocate output frame ranges → optionally leave unallocated gaps or merge adjacent outputs → reorder/remove output episodes → apply the plan → select output directory and encoding settings → press `Start Encode` → create an execution-specific virtual source → build one Clip request per output → confirm overwrite targets → run sequentially with `RepartOutputSidebarPanel` → mux each encoded video into the final MKV, with audio handled by the configured repart `AudioMuxMode`.
...
- `Commands/OpenRawAnalysisCmd.cs` — Raw JSON open
...
- `Commands/OpenClose/OpenQueueEditorCmd.cs` — Source queue ordering modal
