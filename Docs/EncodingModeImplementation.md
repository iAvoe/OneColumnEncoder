...
**Repart:** `SrcRepart` selected → import a folder → strictly analyze sources in import order → open `RepartConfModal` → manually allocate output frame ranges → optionally leave unallocated gaps or merge adjacent outputs → apply the plan → order output episodes in `QueueEditorModal` → select output directory and encoding settings → press `Start Encode` → create an execution-specific virtual source → build one Clip request per output → confirm overwrite targets → run sequentially with `RepartOutputSidebarPanel` → mux each encoded video into the final MKV, with audio handled by the configured repart `AudioMuxMode`.
