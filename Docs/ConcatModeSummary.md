...
`AnalyzeSrcVideoCmd` has a concat branch. It requires at least two selected fragments, analyzes every selected fragment with ffprobe, and only uses a concat total frame count when every fragment already provides a real frame count.
...
This ensures that encoder-side frame limits, encoding progress percentage, and frame-count display reflect the true total across all concatenated fragments. When the total cannot be trusted, the monitor leaves progress unavailable instead of showing a guessed value.
